...
apt-get -qq update
apt-get -qq install -y --no-install-recommends python-dev build-essential sudo git vim dh-autoreconf ninja-build ca-certificates valgrind && apt-get -qq autoremove -y && apt-get -qq clean -y && rm -rf /var/lib/apt/lists/*
